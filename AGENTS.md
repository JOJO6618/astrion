...
- **构建沙箱已知问题（2026-09 实测）**：vite 构建的 PostCSS 配置搜索会向上冒泡读工作区外文件（如工作区父目录的 `package.json`），宿主机沙箱内被 EPERM 拒绝导致 build 失败（tsc/stylelint 前置步骤正常）。这不是代码问题，禁止通过修改构建配置绕过；需要完整构建时请用户在有权限的环境执行，或把对应文件加入路径授权（仅可读）。
