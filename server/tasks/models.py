...
# ---- 任务公开载荷序列化（核心层唯一实现；Web 路由与公共入口共用，保证多端同构）----


def conversation_title_for_task(rec: TaskRecord) -> Optional[str]:
...
def workspace_label_for_task(rec: TaskRecord) -> Optional[str]:
    try:
        if rec.username == "host":
...
def task_public_payload(rec: TaskRecord, *, include_title: bool = True) -> Dict[str, Any]:
    payload = {
        "task_id": rec.task_id,
        "username": rec.username,
        "workspace_id": rec.workspace_id,
        "workspace_label": workspace_label_for_task(rec),
        "status": rec.status,
    ...
    if include_title:
        payload["conversation_title"] = conversation_title_for_task(rec)
    return payload
