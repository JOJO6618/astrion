"""Chat Completions ↔ Codex Responses API 双向映射（纯函数，可单测）。

映射规则详见设计文档 §3（translate 层）。要点：
- 请求：system 并入 instructions；assistant 的 ``codex_reasoning_items`` 原样回插；
  ``reasoning_content``（摘要）**忽略不回传**；孤儿 function_call_output 降级为
  普通 user 文本；tools 拍平（可选参数改 nullable 联合类型，见 flatten_tools）；
  采样参数丢弃；所有 item 不携带服务端 id。
- 响应：Responses 语义化 SSE 事件流 → Chat 增量 chunk 形状，
  ...
  参数在 output_item.done 落定时一次性下发并剥除 null 填充——codex 后端
  约束解码会强制填充所有 schema 属性（实测 + OmniRoute #6951 同款），
...
class CodexAPIError(Exception):
    """Codex 后端返回的业务错误（response.failed / error 事件）。"""
...
def _optional_params_nullable(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    背景（2026-09 实测 + OmniRoute #6951 同款）：codex 后端对函数调用使用
    约束解码，schema 里出现的属性模型必须逐个填充、无法省略，不想填的只能
    塞零值（""/0/[]）。按官方 structured-outputs 惯例提供 null 通道后，
    模型会用 null 表达「不需要」；响应侧由 :func:`sanitize_tool_arguments`
    把 null 剥除，恢复工具声明的可选语义。仅处理顶层 properties。
    """
...
def sanitize_tool_arguments(
    ...
    """剥除「非 required 且值为 null」的参数（codex 约束解码的 null 填充）。
...
def build_responses_body(
    *,
    ...
    """组装 Codex Responses 请求体（含全部强制约束）。"""
...
async def responses_sse_to_chat_chunks(
    ...
    """把 Codex Responses 的 SSE 行流翻译为 Chat 形状 chunk。

    与 ``chat()`` 产出的 chunk 完全同形；遇到 response.failed/error 事件
    抛 :class:`CodexAPIError`（由 mixin 统一转 error chunk）。
    ...
    async for line in lines:
        ...
        if event == "response.created":
            ...
            raise CodexAPIError(
                str(error.get("message") or "Codex 响应失败"),
                code=error.get("code"),
            )

        elif event == "error":
            raise CodexAPIError(
                str(data.get("message") or data.get("error") or "Codex 流式错误"),
                code=data.get("code"),
            )

    if not got_completed:
        # 流在未收到 completed 时结束：视为断流，由上层按重试策略处理
        raise CodexAPIError("Codex 响应流中断（未收到 completed 事件）", code="stream_incomplete")
