...
/**
 * 个人空间共享上下文 composable（从 PersonalizationDrawer.vue 抽离）。
 *
 * 用途：PersonalizationDrawer（个人空间抽屉）与 SettingsShell（全屏设置页）
 * 各自在 setup 中调用本函数，向自己的子树 provide 同一份 'personalizationDrawer' 上下文，
 * 使 tabs/*.vue 子组件在两个宿主中零改动复用（inject('personalizationDrawer') 解构同名绑定）。
 *
 * 宿主差异通过参数注入：
 * - activeTab：宿主当前激活的 tab/section（Drawer 传 personalization store 的 activeTab，
 *   设置页传 settings store 的 activeSection），用于 tab 懒加载触发。
 * - hostVisible：宿主是否可见（Drawer 传 visible；设置页由 App.vue v-if 挂载，传常量 true ref）。
 *
 * 管理员/host 判定统一收敛到 stores/settings.ts（isAdmin/isHostMode/showMcpConfigEntry），
 * 两个宿主共享同一份会话状态（fetchSessionStatus 内部去重）。
 */
export interface PersonalizationContextOptions {
  /** 宿主当前激活的 tab/section（受控 ref；本 composable 只读不写） */
  activeTab: Ref<string>;
  /** 宿主是否可见/激活（用于按需加载触发） */
  hostVisible: Ref<boolean>;
}

export const PERSONALIZATION_DRAWER_CONTEXT_KEY = 'personalizationDrawer';

export function usePersonalizationContext(options: PersonalizationContextOptions) {
  const { activeTab, hostVisible } = options;
  ...
  // ---- 管理员 / host 判定（统一走 settings store，与原抽屉逻辑一致） ----
  const isAdmin = computed(() => settingsStore.isAdmin);
  const showMcpConfigEntry = computed(() => settingsStore.showMcpConfigEntry);
  ...
  // 宿主组件 setup 时检查一次语音模型状态（Android bridge 同步读取，开销可忽略）
  checkVoiceModel();
  ...
  const runModeOptions: Array<{
    ...
    {
      id: 'fast',
      labelKey: 'personalization.runModeFast',
      descKey: 'personalization.runModeFastDesc',
      value: 'fast'
    },
    {
      id: 'thinking',
      labelKey: 'personalization.runModeThinking',
      descKey: 'personalization.runModeThinkingDesc',
      value: 'thinking'
    }
  ];
  ...
  const reasoningEffortOptions: Array<{
    ...
    {
      id: 'default',
      labelKey: 'personalization.effortDefault',
      descKey: 'personalization.effortDefaultDesc',
      value: null
    },
    {
      id: 'low',
      labelKey: 'personalization.effortLow',
      descKey: 'personalization.effortLowDesc',
      value: 'low'
    },
    {
      id: 'medium',
      labelKey: 'personalization.effortMedium',
      descKey: 'personalization.effortMediumDesc',
      value: 'medium'
    },
    {
      id: 'high',
      labelKey: 'personalization.effortHigh',
      descKey: 'personalization.effortHighDesc',
      value: 'high'
    },
    {
      id: 'xhigh',
      labelKey: 'personalization.effortXhigh',
      descKey: 'personalization.effortXhighDesc',
      value: 'xhigh'
    },
    {
      id: 'max',
      labelKey: 'personalization.effortMax',
      descKey: 'personalization.effortMaxDesc',
      value: 'max'
    }
  ];
  ...
  const permissionModeOptions: Array<{
  ...
  }> = [
    {
      ...
      descKey: 'personalization.permissionReadonlyDesc'
    },
    {
      id: 'approval',
      labelKey: 'personalization.permissionApproval',
      descKey: 'personalization.permissionApprovalDesc'
    },
    {
      id: 'auto_approval',
    ...
    {
      id: 'unrestricted',
      labelKey: 'personalization.permissionUnrestricted',
      descKey: 'personalization.permissionUnrestrictedDesc'
    }
  ];
  ...
  const workModeOptions: Array<{
    ...
    {
      id: 'plan',
      labelKey: 'personalization.workModePlan',
      descKey: 'personalization.workModePlanDesc'
    },
    {
      id: 'ask',
      labelKey: 'personalization.workModeAsk',
      descKey: 'personalization.workModeAskDesc'
    },
    {
      id: 'execute',
      labelKey: 'personalization.workModeExecute',
      descKey: 'personalization.workModeExecuteDesc'
    }
  ];
  ...
  const filteredModelOptions = computed(() => {
    ...
    return (modelStore.models || []).map((opt: any) => {
      ...
      return {
        ...
        providerId: opt.providerId || undefined,
        providerName: opt.providerName || undefined,
        badge:
          multimodal === 'image,video'
            ? t('personalization.badgeTextOnly')
            : opt.thinkingOnly
              ? t('personalization.badgeThinking')
              : undefined,
  ...
  const imageCompressionOptions = [
    {
      id: 'original',
      labelKey: 'personalization.imageOriginal',
      descKey: 'personalization.imageOriginalDesc'
    },
    {
      id: '1080p',
      labelKey: 'personalization.image1080p',
      descKey: 'personalization.image1080pDesc'
    },
  ...
  const permissionModeLabel = computed(() => {
    void currentLocale.value;
    const found = permissionModeOptions.find(
      (option) => option.id === form.value.default_permission_mode
    );
    return found ? t(found.labelKey) : t('common.unset');
  });
  ...
  const imageCompressionLabel = computed(() => {
    void currentLocale.value;
    const found = imageCompressionOptions.find(
      (option) => option.id === form.value.image_compression
    );
    return found ? t(found.labelKey) : t('common.unset');
  });
  ...
  const communicationStyleLabel = computed(() => {
    void currentLocale.value;
    if (form.value.communication_style === 'human_like')
      return t('personalization.communicationHumanLike');
  ...
  const blockDisplayLabel = computed(() => {
    void currentLocale.value;
    const found = blockDisplayOptions.find(
      (option) => option.value === currentBlockDisplayMode.value
    );
  ...
  const appUpdateStateText = computed(() => {
    ...
    return appHasUpdate.value
      ? t('personalization.appUpdateFound')
      : t('personalization.appUpdateLatest');
  });
  ...
  onMounted(async () => {
    hydrateAppVersionFromBridge();
    // 会话状态（管理员/host 判定）统一由 settings store 拉取（内部去重）
  ...
  const reviewAgentDefs: Array<{ key: ReviewAgentKey; nameKey: string; descKey: string }> = [
    {
      key: 'auto_approval',
      nameKey: 'personalization.reviewAgentAutoApproval',
      descKey: 'personalization.reviewAgentAutoApprovalDesc'
    },
    {
      key: 'goal_review',
      nameKey: 'personalization.reviewAgentGoalReview',
      descKey: 'personalization.reviewAgentGoalReviewDesc'
    },
    {
      key: 'workflow_review',
      nameKey: 'personalization.reviewAgentWorkflowReview',
      descKey: 'personalization.reviewAgentWorkflowReviewDesc'
    }
  ];
  ...
  const updateReviewAgentInt = (
    key: ReviewAgentKey,
    field: 'timeout_seconds' | 'max_rounds' | 'max_command_timeout',
    raw: string,
    lo: number,
    hi: number
  ) => {
  ...
  const handleRoleEditorOverlayPressStart = (event: MouseEvent) => {
    roleEditorOverlayPressActive.value = event.target === event.currentTarget && event.button === 0;
  };

  const handleRoleEditorOverlayPressEnd = (event: MouseEvent) => {
    const shouldClose = roleEditorOverlayPressActive.value && event.target === event.currentTarget;
  ...
  const restoreCompressionDefaults = () => {
    ...
    personalization.updateField({
      key: 'shallow_compress_trigger_tool_calls_interval',
      value: null
    });
  ...
  const localeOptions: Array<{ id: LocaleKey; labelKey: string; descKey: string }> = [
    {
      id: 'zh-CN',
      labelKey: 'personalization.localeZhCN',
      descKey: 'personalization.localeChineseDesc'
    },
    {
      id: 'en-US',
      labelKey: 'personalization.localeEnglish',
      descKey: 'personalization.localeEnglishDesc'
    }
  ];
  ...
  provide(PERSONALIZATION_DRAWER_CONTEXT_KEY, drawerContext);

  return drawerContext;
}

/** 上下文类型（宿主组件可将返回值类型用于子组件 inject 提示） */
export type PersonalizationDrawerContext = ReturnType<typeof usePersonalizationContext>;
